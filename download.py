...
DOWNLOAD_DIR = "audio_files/ft_news_briefing_episodes" # Changed directory name slightly
NUM_EPISODES = 10
...
def get_episode_audio_url(entry: Dict[str, Any]) -> Optional[str]:
    ...
    logging.warning(f"Could not find audio enclosure for: {entry.get('title', 'Unknown Title')}")
    return None
    ...
    def get_published_datetime(entry: Dict[str, Any]) -> datetime:
        ...
        if 'published_parsed' in entry and entry.published_parsed:
            try:
                ...
                logging.warning(f"Could not parse date for '{entry.get('title', 'Unknown')}': {e}. Using default.")
                return default_date
             ...
             try:
                 ...
                 try:
                     return datetime.strptime(entry.published, "%a, %d %b %Y %H:%M:%S %Z") # Alternative format
                 except ValueError as e_str:
                     logging.warning(f"Could not parse date string '{entry.published}' for '{entry.get('title', 'Unknown')}': {e_str}. Using default.")
                     return default_date
        return default_date
...
async def download_episode(
    ...
    try:
        ...
        async with session.get(audio_url, headers=headers, timeout=300) as response: # Longer timeout for downloads
            if response.status != 200:
                logging.error(f"Download failed for '{title}': Status {response.status}")
                return title, False
            ...
            logging.info(f"Finished download: '{title}' ({downloaded_size / 1024 / 1024:.2f} MB)")
            return title, True
    except asyncio.TimeoutError:
        ...
        for entry in sorted_entries[:NUM_EPISODES]:
            ...
            if audio_url:
                episodes_to_download.append({
                    ...
                    'audio_url': audio_url,
                    'published_parsed': entry.get('published_parsed'), # Keep for reference
                    'id': entry.get('id', entry.get('link', audio_url))
                })
            else:
                 logging.warning(f"Could not get audio URL for: {entry.get('title', 'Unknown Title')}")

        if not episodes_to_download:
            logging.warning("No episodes found with valid audio URLs among the latest.")
            return

        logging.info(f"Found {len(episodes_to_download)} latest episodes with audio URLs to download.")

        results = []
        for i, episode_data in enumerate(episodes_to_download):
            logging.info(f"--- Attempting episode {i+1}/{len(episodes_to_download)} ---")
            ...
            if i < len(episodes_to_download) - 1: # Don't sleep after the last one
                 logging.info(f"Waiting for {DOWNLOAD_DELAY_SECONDS} seconds before next download...")
...
async def download_file():
    ...
    url = "https://audio.listennotes.com/e/p/9dd0289b805f43acac79da115704d726/?_gl=1*1a91oq*_gcl_au*NDA0NzM5MC4xNzQ2MzgwMzI5*_ga*ODYxOTIwODYyLjE3NDYzODA1OTY.*_ga_T0PZE2Z7L4*MTc0NjM4MDU5Ni4xLjEuMTc0NjM4MDYxMy40My4wLjA."

    path = 'C:\\dev\\podsights\\manage_podcasts\\audio_files\\ft_news_briefing_episodes\\' + title

    await download_file_async(url, path)
